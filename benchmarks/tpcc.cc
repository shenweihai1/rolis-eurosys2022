...
//      thread_local static ZipfDist d(g_item_zipf_co/100.0, NumItems());
  ...
  try {
    ...
    bool ok = tbl_customer(warehouse_id)->get(txn, EncodeK(obj_key0, k_c), obj_v);
    //printf("W %d -D %d -C %d -O %d\n", warehouse_id, districtID, customerID, ok) ;
    ALWAYS_ASSERT(ok);
...
public:
  tpcc_bench_runner(abstract_db *db, bool failure=false)
    : bench_runner(db)
  {
    if (failure) {  // failure happens, we reinitialize partitions
        printf("reinitializing partitions under failure\n");
        string nCount[12] = {"customer", "customer_name_idx", "district", "history", "item", "new_order",
                             "oorder", "oorder_c_id_idx", "order_line", "stock", "stock_data", "warehouse"};
...
tpcc_do_test(abstract_db *db, int argc, char **argv)
{
  ...
  while (1) {
    static struct option long_options[] =
    {
      ...
      {"cpu-gap"                              , required_argument , 0                                     , 'g'} ,
      {"item-zipf"                            , required_argument , &g_item_zipf_co                       , 1} ,
      {0, 0, 0, 0}
    };
    int option_index = 0;
    ...
    case 'n':
      num_cpus = strtoul(optarg, NULL, 10);
      ALWAYS_ASSERT(num_cpus > 0);
      break;

    case 'g':
      cpu_gap = strtoul(optarg, NULL, 10);
      ALWAYS_ASSERT(cpu_gap > 0);
      break;
  ...
  #if defined(CPU_PROFILING_FEATURE)
  arr = new char[20];
    sprintf (arr,"Thread_%s","1");
  ProfilerStart(arr);
  #endif
  r.num_cpus = num_cpus;
  r.cpu_gap = cpu_gap;
...
bench_runner*
tpcc_do_test_run(abstract_db *db, int argc, char **argv)
{
    char *arr;
  // parse options
  optind = 1;
  ...
  if (f_mode == 1) {
      r = new tpcc_bench_runner(db, true);
  } else {
      r = new tpcc_bench_runner(db);
  }
  //tpcc_bench_runner *r = new tpcc_bench_runner(db);
  #if defined(CPU_PROFILING_FEATURE)
  arr = new char[20];
    sprintf (arr,"Thread_%s","1");
  ProfilerStart(arr);
  ...
  r->num_cpus = num_cpus;
  r->cpu_gap = cpu_gap;
  r->f_mode = f_mode;  // run with loaders and workers
  r->run_without_stats();
  #if defined(CPU_PROFILING_FEATURE)
    ProfilerStop();
    free (arr);
