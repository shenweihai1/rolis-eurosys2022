...
public:
    rsimple_worker(unsigned int worker_id,
    ...
    {
        return static_cast<rsimple_worker *>(w)->txn_read();
    }
    ...
    txn_write()
    {
        ...
        try {
            ...
            auto s2 = str().assign(RSimpleRecordSize, 'b');
            tbl->put(txn, s, s2);
    ...
    {
        return static_cast<rsimple_worker *>(w)->txn_write();
    }
    ...
    txn_rmw()
    {
        ...
        try {
            for (int i=0; i<RMW_count; i++) {
                ...
                computation_n += obj_v.size();
                tbl->put(txn, obj_key0, str().assign(RSimpleRecordSize, 'c'));
            }
    ...
    {
        return static_cast<rsimple_worker *>(w)->txn_scan();
    }
...
class rsimple_usertable_loader : public bench_loader {
public:
    rsimple_usertable_loader(unsigned long seed,
                          abstract_db *db,
                          const map<string, abstract_ordered_index *> &open_tables)
...
class rsimple_parallel_usertable_loader : public bench_loader {
public:
    rsimple_parallel_usertable_loader(unsigned long seed,
                                   abstract_db *db,
                                   const map<string, abstract_ordered_index *> &open_tables,
                                   unsigned int pinid,
                                   uint64_t keystart,
              ...
              pinid(pinid), keystart(keystart), keyend(keyend)
    {
        ...
        if (verbose)
            cerr << "[INFO] RSimple par loader cpu " << pinid
...
class rsimple_bench_runner : public bench_runner {
public:
    rsimple_bench_runner(abstract_db *db)
            : bench_runner(db)
    {
        open_tables["USERTABLE"] = db->open_index("USERTABLE", RSimpleRecordSize);
    }
        ...
        if (enable_parallel_loading && nkeys >= nthreads) {
            ...
            if (nthreads > ncpus) {
                for (size_t i = 0; i < ncpus; i++) {
                    ...
                    ret.push_back(
                            new rsimple_parallel_usertable_loader(
                                    0, db, open_tables, i,
            ...
            ret.push_back(new rsimple_usertable_loader(0, db, open_tables));
        }
        return ret;
    }
...
bench_runner*
rsimple_do_test(abstract_db *db, int argc, char **argv)
{
    nkeys = size_t(scale_factor * 1000.0);
    nkeys = 100 * 10000 ; // 1 million
    //nkeys = 10000 ;
    ALWAYS_ASSERT(nkeys > 0);
